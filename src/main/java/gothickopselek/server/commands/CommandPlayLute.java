			...
			if(Integer.parseInt(args[0]) < 10 && Integer.parseInt(args[0]) > 0)
			{
				if(sender instanceof EntityPlayer)
				{
					...
					if(p.getHeldItemOffhand().getItem() == ModItems.lute)
					{
						...
						p.getServer().getEntityWorld().playSound(null, pPos.getX(), pPos.getY(), pPos.getZ(), SoundEvents.BLOCK_NOTE_GUITAR, SoundCategory.AMBIENT, 2.0F, 0.05F + (Float.parseFloat(args[0]) / 5));
					}
				}
			}
		}
